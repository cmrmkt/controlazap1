...
CREATE POLICY "authenticated_users_can_update_own_categories" ON public.categorias
    ...
    WITH CHECK (
        auth.uid() = userid AND
        -- Impedir alteração do proprietário
        userid = OLD.userid
    );
...
CREATE POLICY "authenticated_users_can_update_own_transactions" ON public.transacoes
    ...
    WITH CHECK (
        auth.uid() = userid AND
        user_owns_category(category_id) AND
        -- Impedir alteração do proprietário
        userid = OLD.userid
    );
...
-- =====================================================
-- 9. FUNÇÃO DE SEGURANÇA ADICIONAL
-- =====================================================

-- Função para verificar propriedade de dados
...
COMMENT ON FUNCTION public.verify_user_ownership(text, text, text) IS 'Função segura para verificar propriedade de registros por usuário';

-- Comentários nas políticas principais
COMMENT ON POLICY "authenticated_users_can_view_own_profile" ON public.profiles IS 'Usuários autenticados podem visualizar apenas seu próprio perfil';
COMMENT ON POLICY "authenticated_users_can_view_own_categories" ON public.categorias IS 'Usuários autenticados podem visualizar apenas suas próprias categorias';
COMMENT ON POLICY "authenticated_users_can_view_own_transactions" ON public.transacoes IS 'Usuários autenticados podem visualizar apenas suas próprias transações';
COMMENT ON POLICY "authenticated_users_can_view_own_reminders" ON public.lembretes IS 'Usuários autenticados podem visualizar apenas seus próprios lembretes';